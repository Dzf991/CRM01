	...
	$(function(){
		...
		$("#openSearchModal").click(function () {
			...
			$("#aname").keydown(function (event) {
				if (event.keyCode === 13){
					$.ajax({
						url:"workbench/clue/searchActivity",
						type:"get",
						data:{
							"name":$("#aname").val()
						},
			...
			$("#checkBtn").click(function () {
				var checkId = $("[name='activity']:checked").val();
				var activityName = $("#"+checkId).html();

				$("#activityName").val(activityName);
				$("#activityId").val(checkId);
			})
		...
		$("#convertBtn").click(function () {
			if ($("#isCreateTransaction").prop("checked")){
				...
				window.location.href="workbench/clue/convert?clueId=${param.clueId}";
			}
		})
	});
</script>
...
<%--							</tr>--%>
				...
				<div class="modal-footer">
					...
					<button type="button" id="checkBtn" class="btn btn-primary" data-dismiss="modal">选择</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div id="create-transaction2" style="position: relative; left: 40px; top: 20px; width: 80%; background-color: #F7F7F7; display: none;" >
		...
		<form action="workbench/clue/convert" id="transForm" method="post">
